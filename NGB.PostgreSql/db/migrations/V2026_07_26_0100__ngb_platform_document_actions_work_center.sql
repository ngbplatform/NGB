...
CREATE TABLE IF NOT EXISTS platform_document_action_executions (
    ...
    CONSTRAINT ck_platform_document_action_execution_code CHECK (
        action_code = lower(trim(action_code)) AND action_code ~ '^[a-z0-9._:-]+$'
    ),
...
CREATE TABLE IF NOT EXISTS platform_outbox_events (
    ...
    source varchar(100) NOT NULL CHECK (length(trim(source)) BETWEEN 1 AND 100),
    subject varchar(300) NOT NULL CHECK (length(trim(subject)) BETWEEN 1 AND 300),
...
CREATE TABLE IF NOT EXISTS platform_outbox_consumer_state (
    ...
    consumer_code varchar(128) NOT NULL CHECK (length(trim(consumer_code)) BETWEEN 1 AND 128),
    ...
    CONSTRAINT ck_platform_outbox_completion CHECK ((status = 3 AND completed_at_utc IS NOT NULL) OR status <> 3)
);
...
CREATE TABLE IF NOT EXISTS platform_notifications (
    ...
    title varchar(300) NOT NULL CHECK (length(trim(title)) BETWEEN 1 AND 300),
...
CREATE TABLE IF NOT EXISTS platform_tasks (
    ...
    priority smallint NOT NULL CHECK (priority IN (1, 2, 3, 4)),
    ...
    CONSTRAINT ck_platform_tasks_assignment CHECK (
        ...
        OR (assigned_user_id IS NULL AND assigned_role_id IS NOT NULL)
    ),
    CONSTRAINT ck_platform_tasks_claim CHECK (claimed_by_user_id IS NULL OR assigned_role_id IS NOT NULL),
...
CREATE TABLE IF NOT EXISTS platform_user_notification_preferences (
    ...
    channel smallint NOT NULL CHECK (channel = 1),
    is_enabled boolean NOT NULL,
    updated_at_utc timestamptz NOT NULL,
    version bigint NOT NULL DEFAULT 1 CHECK (version > 0),
    PRIMARY KEY (user_id, notification_code, channel),
    CONSTRAINT ck_platform_notification_preferences_code CHECK (
        notification_code = lower(trim(notification_code)) AND notification_code ~ '^[a-z0-9._:-]+$'
    )
);
