...
public sealed class TradeCommandPaletteSearchService(
    IDocumentService documents,
    ICatalogService catalogs,
    IReportDefinitionProvider reports,
    IMemoryCache cache,
    ILogger<TradeCommandPaletteSearchService> logger)
{
        ...
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(ex, "Trade command palette provider {ProviderCode} failed.", providerCode);
            return null;
        }
    }
    ...
    private async Task<IReadOnlyList<SearchableDescriptor>> GetDocumentDescriptorsAsync(CancellationToken ct)
        => await cache.GetOrCreateAsync(
               ...
               async entry =>
               {
                   ...
                   return metadata
                       .Where(static item =>
                           item.DocumentType.StartsWith("trd.", StringComparison.OrdinalIgnoreCase)
    ...
    private async Task<IReadOnlyList<SearchableDescriptor>> GetCatalogDescriptorsAsync(CancellationToken ct)
        => await cache.GetOrCreateAsync(
               ...
               async entry =>
               {
                   ...
                   return metadata
                       .Where(static item => item.CatalogType.StartsWith("trd.", StringComparison.OrdinalIgnoreCase))
                       .Select(item => new SearchableDescriptor(
                           item.CatalogType,
                           item.DisplayName,
               ...
               async entry =>
               {
                   ...
                   return definitions
                       .Where(static definition =>
                           definition.ReportCode.StartsWith("trd.", StringComparison.OrdinalIgnoreCase)
    ...
    private static string[] ResolveAliases(string code, string label)
        => code switch
        {
            _ when string.Equals(code, TradeCodes.Item, StringComparison.OrdinalIgnoreCase)
                => ["item", "items", "product", "products", "sku", label],
            _ when string.Equals(code, TradeCodes.Warehouse, StringComparison.OrdinalIgnoreCase)
                => ["warehouse", "stock", "location", label],
            _ when string.Equals(code, TradeCodes.Party, StringComparison.OrdinalIgnoreCase)
                => ["party", "customer", "vendor", "partner", label],
            _ when string.Equals(code, TradeCodes.PriceType, StringComparison.OrdinalIgnoreCase)
                => ["price", "price type", "pricing", label],
            _ when string.Equals(code, TradeCodes.PurchaseReceipt, StringComparison.OrdinalIgnoreCase)
                => ["purchase", "receipt", "receive stock", label],
            _ when string.Equals(code, TradeCodes.SalesInvoice, StringComparison.OrdinalIgnoreCase)
                => ["sales", "invoice", "sale", label],
            _ when string.Equals(code, TradeCodes.InventoryTransfer, StringComparison.OrdinalIgnoreCase)
                => ["inventory", "transfer", "movement", label],
            _ when string.Equals(code, TradeCodes.InventoryAdjustment, StringComparison.OrdinalIgnoreCase)
                => ["inventory", "adjustment", label],
            _ when string.Equals(code, TradeCodes.CustomerReturn, StringComparison.OrdinalIgnoreCase)
                => ["customer", "return", label],
            _ when string.Equals(code, TradeCodes.VendorReturn, StringComparison.OrdinalIgnoreCase)
                => ["vendor", "return", label],
            _ when string.Equals(code, TradeCodes.CustomerPayment, StringComparison.OrdinalIgnoreCase)
                => ["customer", "payment", label],
            _ when string.Equals(code, TradeCodes.VendorPayment, StringComparison.OrdinalIgnoreCase)
    ...
    private static string ResolveReportIcon(string reportCode)
        => reportCode switch
        {
            TradeCodes.InventoryBalancesReport => "bar-chart",
            TradeCodes.InventoryMovementsReport => "git-merge",
            TradeCodes.CurrentItemPricesReport => "tag",
            TradeCodes.SalesByItemReport or TradeCodes.SalesByCustomerReport => "bar-chart",
            TradeCodes.PurchasesByVendorReport => "bar-chart",
