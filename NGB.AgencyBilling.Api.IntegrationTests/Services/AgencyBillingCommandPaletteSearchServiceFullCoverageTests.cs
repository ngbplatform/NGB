...
public sealed class AgencyBillingCommandPaletteSearchServiceFullCoverageTests
{
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    ...
    public async Task SearchAsync_AllProviders_ReturnsRankedDomainResultsAndCachesMetadata()
    {
        ...
        fixture.SetReports(AllReportIconCodes()
            ...
            .Append(new ReportDefinitionDto(AgencyBillingCodes.DashboardOverviewReport, "Match empty description", null, " "))
            .Append(new ReportDefinitionDto("ab.custom", "Match custom"))
        ...
        fixture.SetDocumentHits(
            new DocumentLookupDto(documentId, AgencyBillingCodes.Client, "Match display", DocumentStatus.Draft, false, "  MATCH-001  "),
            new DocumentLookupDto(fallbackDocumentId, AgencyBillingCodes.TeamMember, "   ", DocumentStatus.Posted, false, "match-posted"),
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, "match same", DocumentStatus.Draft, false, "match same"),
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, "match empty number", DocumentStatus.Draft, false, "   "),
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Project, "Match party", DocumentStatus.MarkedForDeletion, true),
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.RateCard, "Match price", (DocumentStatus)99, false),
            new DocumentLookupDto(Guid.NewGuid(), "ab.unknown", "Match unknown", DocumentStatus.Draft, false));
        fixture.SetCatalogHits(
            new CatalogLookupDto(catalogId, AgencyBillingCodes.Client, " Match catalog ", false),
            new CatalogLookupDto(Guid.NewGuid(), AgencyBillingCodes.TeamMember, "Match deleted", true),
            new CatalogLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, null, false),
            new CatalogLookupDto(Guid.NewGuid(), AgencyBillingCodes.Project, "   ", false),
            new CatalogLookupDto(Guid.NewGuid(), "ab.unknown", "Match unknown", false));

        var context = new CommandPaletteSearchContextDto(
            EntityType: "other",
            DocumentType: AgencyBillingCodes.Client.ToUpperInvariant(),
            CatalogType: AgencyBillingCodes.Client.ToUpperInvariant());
        ...
        first.Groups.Single(x => x.Code == "documents").Items.Should().Contain(x =>
            x.Key == $"document:{AgencyBillingCodes.Client}:{documentId}"
            && x.Title == "ab.client MATCH-001"
            && x.Subtitle == "Match display · Draft"
            && x.Icon == "custom-icon"
            && x.Status == "draft"
            && x.Score == 0.95m);
        first.Groups.Single(x => x.Code == "documents").Items.Should().Contain(x =>
            x.Key == $"document:{AgencyBillingCodes.TeamMember}:{fallbackDocumentId}"
            && x.Subtitle == "Posted");
        ...
        first.Groups.Single(x => x.Code == "catalogs").Items.Should().Contain(x =>
            x.Key == $"catalog:{AgencyBillingCodes.Client}:{catalogId}"
    ...
    {
        using var fixture = new Fixture();
        fixture.SetDocumentMetadata(DocumentMetadata(AgencyBillingCodes.Client));
        fixture.SetDocumentHits(new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, display, DocumentStatus.Draft, false));
    ...
    {
        using var fixture = new Fixture();
        fixture.SetDocumentMetadata(DocumentMetadata(AgencyBillingCodes.Client, label: "Customer"));
        fixture.SetDocumentHits(
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, null, DocumentStatus.Draft, false),
            new DocumentLookupDto(Guid.NewGuid(), AgencyBillingCodes.Client, null, DocumentStatus.Draft, false, " "));
        fixture.SetCatalogMetadata(CatalogMetadata(AgencyBillingCodes.Client));
        var catalogId = Guid.NewGuid();
        fixture.SetCatalogHits(new CatalogLookupDto(catalogId, AgencyBillingCodes.Client, " ", false));
        fixture.SetReports(new ReportDefinitionDto(AgencyBillingCodes.DashboardOverviewReport, "client", " ", null));

        var result = await fixture.Sut.SearchAsync(new CommandPaletteSearchRequestDto("client"), CancellationToken.None);

        result.Groups.Single(x => x.Code == "documents").Items.Should().OnlyContain(x =>
            x.Title.StartsWith("Customer ", StringComparison.Ordinal));
        result.Groups.Single(x => x.Code == "catalogs").Items.Single().Title.Should().Be($"{AgencyBillingCodes.Client} {catalogId}");
    ...
    public async Task SearchAsync_NonMatchingReport_ReturnsNoGroup()
    {
        ...
        fixture.SetReports(new ReportDefinitionDto(
            AgencyBillingCodes.DashboardOverviewReport,
            "needle",
            " ",
            "description"));
    ...
    public async Task SearchAsync_NonPositiveLimit_UsesDefault_AndSkipsNonMatchingCatalogHits()
    {
        using var fixture = new Fixture();
        fixture.SetCatalogMetadata(CatalogMetadata(AgencyBillingCodes.Client));
        fixture.SetCatalogHits(new CatalogLookupDto(Guid.CreateVersion7(), AgencyBillingCodes.Client, "unrelated", false));
    ...
    public async Task SearchAsync_ProviderFailureIsIsolatedAndUncancelledCancellationIsLogged()
    {
        ...
        fixture.SetReports(new ReportDefinitionDto(AgencyBillingCodes.DashboardOverviewReport, "match"));
    ...
    {
        yield return AgencyBillingCodes.Client;
        yield return AgencyBillingCodes.TeamMember;
        yield return AgencyBillingCodes.Project;
        yield return AgencyBillingCodes.RateCard;
        yield return AgencyBillingCodes.ServiceItem;
        yield return AgencyBillingCodes.SalesInvoice;
        yield return AgencyBillingCodes.ClientContract;
        yield return AgencyBillingCodes.Timesheet;
        yield return AgencyBillingCodes.CustomerPayment;
        yield return AgencyBillingCodes.PaymentTerms;
    }
    ...
    {
        yield return AgencyBillingCodes.DashboardOverviewReport;
        yield return AgencyBillingCodes.UnbilledTimeReport;
        yield return AgencyBillingCodes.ProjectProfitabilityReport;
        yield return AgencyBillingCodes.InvoiceRegisterReport;
        yield return AgencyBillingCodes.ArAgingReport;
        yield return AgencyBillingCodes.TeamUtilizationReport;
    }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        public Fixture()
        {
            ...
            Sut = new AgencyBillingCommandPaletteSearchService(
                Documents.Object,
                Catalogs.Object,
                Reports.Object,
                _cache,
        ...
        public AgencyBillingCommandPaletteSearchService Sut { get; }
