...
internal static class PropertyManagementSeedCliArgs
{
    ...
    public static double GetDouble(string[] args, string name, double defaultValue)
    {
        ...
        if (!double.TryParse(raw, NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out var value))
            throw new NgbArgumentInvalidException(name, $"'{name}' must be a valid number.");

        return value;
    }

    public static string GetString(string[] args, string name, string defaultValue)
    {
        var raw = GetArgValue(args, name);
