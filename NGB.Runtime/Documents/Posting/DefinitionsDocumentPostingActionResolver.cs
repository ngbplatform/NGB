using NGB.Accounting.Posting;
using NGB.Core.Documents;
using NGB.Core.Documents.Exceptions;
using NGB.Definitions;
using NGB.Definitions.Documents.Posting;
using NGB.Runtime.Internal;
using NGB.Tools.Exceptions;

namespace NGB.Runtime.Documents.Posting;

/// <summary>
/// Resolves posting handlers from <see cref="DefinitionsRegistry"/> and builds a posting delegate.
/// </summary>
public sealed class DefinitionsDocumentPostingActionResolver(
    DefinitionsRegistry definitions,
    IEnumerable<IDocumentPostingHandler> handlers)
    : IDocumentPostingActionResolver
{
    private readonly DefinitionsRegistry _definitions = definitions ?? throw new NgbArgumentRequiredException(nameof(definitions));
    private readonly IReadOnlyList<IDocumentPostingHandler> _allHandlers = DefinitionRuntimeBindingHelpers.ToReadOnlyList(
        handlers ?? throw new NgbArgumentRequiredException(nameof(handlers)));
    private readonly Dictionary<string, IDocumentPostingHandler> _handlersByTypeCode = new(StringComparer.OrdinalIgnoreCase);
    private readonly Lock _handlersGate = new();

    public Func<IAccountingPostingContext, CancellationToken, Task>? TryResolve(DocumentRecord document)
    {
        if (document is null)
            throw new NgbArgumentRequiredException(nameof(document));

        if (!_definitions.TryGetDocument(document.TypeCode, out var def))
            throw new DocumentTypeNotFoundException(document.TypeCode);

        if (def.PostingHandlerType is null)
            return null;

        var handler = ResolveHandler(def);

        return (ctx, ct) => handler.BuildEntriesAsync(document, ctx, ct);
    }

    public Func<IAccountingPostingContext, CancellationToken, Task> Resolve(DocumentRecord document)
        => TryResolve(document)
           ?? throw new DocumentPostingHandlerNotConfiguredException(document.Id, document.TypeCode);

    private IDocumentPostingHandler ResolveHandler(NGB.Definitions.Documents.DocumentTypeDefinition def)
    {
        if (_handlersByTypeCode.TryGetValue(def.TypeCode, out var cached))
            return cached;

        lock (_handlersGate)
        {
            if (_handlersByTypeCode.TryGetValue(def.TypeCode, out cached))
                return cached;

            var resolved = BuildHandler(def);
            _handlersByTypeCode[def.TypeCode] = resolved;
            return resolved;
        }
    }

    private IDocumentPostingHandler BuildHandler(NGB.Definitions.Documents.DocumentTypeDefinition def)
    {
        var postingHandlerType = def.PostingHandlerType
            ?? throw new NgbInvariantViolationException($"Document type '{def.TypeCode}' has no accounting posting handler binding.");
        var postingHandlerTypeName = postingHandlerType.FullName ?? postingHandlerType.Name;

        if (!typeof(IDocumentPostingHandler).IsAssignableFrom(postingHandlerType))
        {
            throw new DocumentPostingHandlerMisconfiguredException(
                def.TypeCode,
                postingKind: "accounting",
                reason: "Posting handler type must implement IDocumentPostingHandler.",
                postingHandlerType: postingHandlerTypeName,
                details: new { documentTypeCode = def.TypeCode, postingHandlerType = postingHandlerTypeName });
        }

        var matches = DefinitionRuntimeBindingHelpers.FindMatches(postingHandlerType, _allHandlers);

        if (matches.Length == 0)
        {
            throw new DocumentPostingHandlerMisconfiguredException(
                def.TypeCode,
                postingKind: "accounting",
                reason: "Posting handler type is not registered in DI container.",
                postingHandlerType: postingHandlerTypeName,
                details: new { documentTypeCode = def.TypeCode, postingHandlerType = postingHandlerTypeName });
        }

        if (matches.Length > 1)
        {
            throw new DocumentPostingHandlerMisconfiguredException(
                def.TypeCode,
                postingKind: "accounting",
                reason: "Multiple posting handler registrations match the configured posting handler type.",
                postingHandlerType: postingHandlerTypeName,
                details: new
                {
                    documentTypeCode = def.TypeCode,
                    postingHandlerType = postingHandlerTypeName,
                    matches = matches.Select(handler => handler.GetType().FullName ?? handler.GetType().Name).ToArray()
                });
        }

        var resolved = matches[0];
        if (!string.Equals(resolved.TypeCode, def.TypeCode, StringComparison.OrdinalIgnoreCase))
        {
            throw new DocumentPostingHandlerMisconfiguredException(
                def.TypeCode,
                postingKind: "accounting",
                reason: $"Posting handler TypeCode does not match. Expected '{def.TypeCode}', actual '{resolved.TypeCode}'.",
                postingHandlerType: postingHandlerTypeName,
                details: new { documentTypeCode = def.TypeCode, postingHandlerType = postingHandlerTypeName });
        }

        return resolved;
    }
}
