...
/// <summary>Complete execution for canonical and dataset-backed reports. The download owns a repeatable-read session.</summary>
    ...
    : IStreamingReportExecutor
{
    ...
    public ReportSheetDto Template(string preparedJson)
        => _template ?? throw new NgbInvariantViolationException("Report template is available after the first row has been read.");
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        {
            var executor = specialized
                .Single(x => string.Equals(x.ReportCode, definition.ReportCode, StringComparison.OrdinalIgnoreCase));
            ...
            do
            {
                ...
                var sheet = page.PrebuiltSheet
                    ?? throw new NgbInvariantViolationException("Canonical executor must supply a report sheet.");
