...
internal sealed class ReceivableChargePayloadValidator(
    ...
    : IDocumentDraftPayloadValidator
{
    ...
    private async Task<Snapshot?> ResolveSnapshotAsync(Guid? documentId, RecordPayload payload, CancellationToken ct)
    {
        var current = documentId is null
            ? null
            : await readers.ReadReceivableChargeHeadAsync(documentId.Value, ct);
        ...
        {
            throw ReceivableChargeValidationException.LeaseNotFound(leaseId.Value, documentId);
        }

        if (leaseDocument.Status == DocumentStatus.MarkedForDeletion)
            throw ReceivableChargeValidationException.LeaseMarkedForDeletion(leaseId.Value, documentId);
