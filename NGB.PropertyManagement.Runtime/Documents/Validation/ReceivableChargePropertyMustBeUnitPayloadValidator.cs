...
/// <summary>
/// Draft-time invariant for pm.receivable_charge:
/// - property_id must reference an existing, non-deleted pm.property with kind = Unit.
///
/// Posting-time validation currently validates lease consistency, but drafts should be kept consistent too
/// (and we want a friendly error on Save, not only on Post).
/// </summary>
internal sealed class ReceivableChargePropertyMustBeUnitPayloadValidator(IPropertyManagementDocumentReaders readers)
