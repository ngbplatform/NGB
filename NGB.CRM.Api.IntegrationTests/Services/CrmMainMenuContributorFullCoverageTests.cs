...
public sealed class CrmMainMenuContributorFullCoverageTests
{
    [Fact]
    public async Task ContributeAsync_AvailableDomainAndExternalItems_IncludesEveryItemKind()
    {
        ...
        [
            new DocumentTypeMetadata(CrmCodes.LeadIntake, [])
        ]);
        var sut = new CrmMainMenuContributor(
            catalogs.Object,
            documents.Object,
        ...
        groups.SelectMany(x => x.Items).Should().Contain(item =>
            item.Kind == "catalog" && item.Code == CrmCodes.Account);
        groups.SelectMany(x => x.Items).Should().Contain(item =>
            item.Kind == "document" && item.Code == CrmCodes.LeadIntake);
        groups.SelectMany(x => x.Items).Should().Contain(item =>
    ...
    public async Task ContributeAsync_OmitsUnavailableDomainAndBlankExternalItems()
    {
        ...
        var sut = new CrmMainMenuContributor(catalogs.Object, documents.Object, new ExternalLinksSettings("", " "));
