...
describe('date values helpers', () => {
  it('parses and normalizes month values across year boundaries', () => {
    ...
    expect(normalizeMonthValue(null)).toBeNull()
    ...
    expect(monthValueYear('bad')).toBeNull()
    expect(shiftMonthValue('2026-01', -1)).toBe('2025-12')
    expect(shiftMonthValue('2026-12', 1)).toBe('2027-01')
    expect(shiftMonthValue('bad', 1)).toBeNull()
    expect(shiftMonthValue('2026-01', Number.POSITIVE_INFINITY)).toBeNull()
    expect(relativeMonthValue(2, new Date(2026, 10, 1))).toBe('2027-01')
    expect(relativeMonthValue(undefined, new Date(2026, 3, 1))).toBe('2026-04')
  })
  ...
  it('converts between month/date-only values and rejects invalid calendar dates', () => {
    ...
    expect(monthValueToDateOnly('bad')).toBeNull()
    expect(dateOnlyToMonthValue('2026-04-30')).toBe('2026-04')
    expect(dateOnlyToMonthValue('bad')).toBeNull()
    expect(normalizeDateOnlyValue(null)).toBeNull()
    expect(normalizeDateOnlyValue('not-a-date')).toBeNull()
    expect(normalizeDateOnlyValue('2026-00-01')).toBeNull()
    expect(normalizeDateOnlyValue('2026-01-00')).toBeNull()
    ...
    expect(parseDateOnlyValue('bad')).toBeNull()
