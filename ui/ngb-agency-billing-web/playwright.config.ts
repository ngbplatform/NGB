...
const e2eHost = process.env.AGENCY_BILLING_WEB_E2E_HOST?.trim() || AGENCY_BILLING_WEB_DEV_HOST
// Keep E2E servers outside the adjacent ports used by running vertical dev servers.
const e2ePort = parsePort(process.env.AGENCY_BILLING_WEB_E2E_PORT, AGENCY_BILLING_WEB_DEV_PORT + 100)
const e2eBaseUrl = `http://${e2eHost}:${e2ePort}`
const workCenterSpecs = /agency-billing-web\/work-center\.spec\.ts/

loadE2eEnv({
  rootDir: uiWorkspaceDir,
  appDirectory: 'ngb-agency-billing-web',
})
...
export default defineConfig({
  ...
  webServer: {
    command: `npm --workspace ngb-agency-billing-web run dev -- --host ${e2eHost} --port ${e2ePort} --strictPort --mode e2e`,
    port: e2ePort,
  ...
  projects: [
    ...
    },
    {
      ...
      testMatch: /agency-billing-web\/.*\.spec\.ts/,
      use: {
