...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    workCenter: { kind: 'work-center' }, lookup: { kind: 'lookup' }, reporting: { kind: 'reporting' },
    editor: { kind: 'editor' }, metadata: { kind: 'metadata' }, palette: { kind: 'palette' },
  }
})
...
vi.mock('../../src/App.vue', () => ({ default: { name: 'AgencyBillingAppStub' } }))
vi.mock('../../src/router/router', () => ({ router: mocks.router }))
vi.mock('../../src/command-palette/config', () => ({ createAgencyBillingCommandPaletteConfig: () => mocks.palette }))
vi.mock('../../src/metadata/framework', () => ({ createAgencyBillingMetadataConfig: () => mocks.metadata }))
vi.mock('../../src/editor/framework', () => ({ createAgencyBillingEditorConfig: () => mocks.editor }))

describe('agency billing application bootstrap', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.window = { location: { pathname: '/documents/ab.timesheet', search: '?status=draft' } } as typeof window
  })
  ...
  it('redirects unauthenticated users to login without loading the application', async () => {
    mocks.auth.authenticated = false
    await import('../../src/main')
    await vi.waitFor(() => expect(mocks.auth.login).toHaveBeenCalledWith('/documents/ab.timesheet?status=draft'))
    expect(mocks.createApp).not.toHaveBeenCalled()
  })

  it('mounts a retry-capable application after initialization failure', async () => {
    mocks.auth.authenticated = false
