...
vi.mock('@ngbplatform/ui', async () => {
  ...
  return {
    NgbCommandPaletteDialog: {
      ...
      render: () => renderNode('div', { 'data-testid': 'ab-palette-dialog' }),
    },
    NgbSiteShell: {
      name: 'NgbSiteShell',
      ...
      setup(
        ...
        return () => renderNode('div', { 'data-testid': 'ab-shell' }, [
          renderNode('div', { 'data-testid': 'ab-shell-title' }, props.moduleTitle ?? ''),
          renderNode('div', { 'data-testid': 'ab-shell-selected-id' }, props.selectedId ?? ''),
          renderNode('div', { 'data-testid': 'ab-shell-email' }, props.userEmail ?? ''),
          renderNode('div', { 'data-testid': 'ab-shell-meta' }, props.userMeta ?? ''),
          renderNode('div', { 'data-testid': 'ab-shell-meta-icon' }, props.userMetaIcon ?? ''),
          renderNode('nav', { 'data-testid': 'ab-shell-nav' }, (props.nodes ?? []).flatMap((node) => [
            renderNode('div', { 'data-testid': 'ab-shell-node' }, node.label),
            ...(node.children ?? []).map((child) => renderNode('div', { 'data-testid': 'ab-shell-node' }, child.label)),
          ])),
          ...
          renderNode('button', { type: 'button', onClick: () => emit('navigate', 'https://status.example/ab') }, 'Navigate external'),
...
function createAuthStore(overrides: Partial<AuthStore> = {}): AuthStore {
  return reactive({
    ...
    redirecting: false,
    authenticated: false,
    userName: 'Agency Tester',
    email: 'agency@example.com',
    primaryRoleLabel: 'Agency Administrator',
  ...
  return await render(App, {
    global: {
      stubs: {
        RouterView: {
          name: 'RouterView',
          render: () => h('div', { 'data-testid': 'ab-router-view' }),
        },
      },
    },
  })
...
test('hydrates a complete shell and handles navigation, palette, and logout actions', async () => {
  ...
  await expect.element(view.getByTestId('ab-shell-title')).toHaveTextContent('Agency Billing')
  await expect.element(view.getByTestId('ab-shell-selected-id')).toHaveTextContent('group:menu')
  await expect.element(view.getByTestId('ab-shell-email')).toHaveTextContent('')
  await expect.element(view.getByTestId('ab-shell-meta-icon')).toHaveTextContent('')
  expect(Array.from(document.querySelectorAll('[data-testid="ab-shell-node"]'), (element) => element.textContent)).toEqual([
  ...
  expect(open).toHaveBeenCalledWith('https://status.example/ab', '_self')
  ...
  expect(view.getByTestId('ab-palette-dialog').element()).not.toBeNull()
  expect(view.getByTestId('ab-router-view').element()).not.toBeNull()
})
...
test('selects an exact child route and handles an empty current path', async () => {
  ...
  const view = await renderApp()
  await expect.element(view.getByTestId('ab-shell-selected-id')).toHaveTextContent('')
  mocks.route.path = '/second'
  mocks.route.fullPath = '/second'
  await expect.element(view.getByTestId('ab-shell-selected-id')).toHaveTextContent('catalog:second')
...
test('renders empty navigation for an absent group collection', async () => {
  ...
  await expect.element(view.getByTestId('ab-shell-nav')).toHaveTextContent('')
  expect(document.querySelectorAll('[data-testid="ab-shell-node"]')).toHaveLength(0)
})
...
test('renders a bare route without shell chrome and palette', async () => {
  ...
  expect(view.getByTestId('ab-router-view').element()).not.toBeNull()
  expect(document.querySelector('[data-testid="ab-shell"]')).toBeNull()
  expect(document.querySelector('[data-testid="ab-palette-dialog"]')).toBeNull()
})
