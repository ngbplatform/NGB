...
const catalogType = 'ab.accounting_policy'
...
const auditEntityTitle = computed(() => String(model.value.display ?? 'Accounting Policy').trim())
      ...
      row.fields = (row.fields ?? [])
        ...
        .map((field) => {
          if (field.key === 'cash_account_id') {
            ...
            field.helpText = 'Fallback cash account used when customer payments do not explicitly choose a settlement account.'
          }

          if (field.key === 'ar_account_id') {
            field.label = 'Accounts Receivable Account'
            field.helpText = 'Control account used when posted sales invoices and customer payments affect receivables.'
          }

          if (field.key === 'service_revenue_account_id') {
            field.label = 'Service Revenue Account'
            field.helpText = 'Default revenue account for posted service billing.'
          }

          if (field.key === 'default_currency') {
            field.label = 'Default Currency'
            field.helpText = 'Base sell-side currency prefilled by setup defaults and commercial documents.'
          }
...
function ensureDisplayValue(): void {
  ...
  if (!value) model.value.display = 'Default Agency Billing Policy'
}

async function copyShareLink(): Promise<void> {
  if (!policy.value?.id) return
  await copyAppLink(router, toasts, { path: '/catalogs/ab.accounting_policy' })
}
...
async function applyDefaults(): Promise<void> {
  ...
  try {
    ...
    error.value = toErrorMessage(cause, 'Failed to apply agency billing default configuration.')
  } finally {
...
<template>
  <div class="flex h-full min-h-0 flex-col" data-testid="agency-billing-accounting-policy-page">
    <NgbPageHeader title="Accounting Policy" can-back @back="router.back()">
      <template #secondary>
        <div class="truncate text-xs text-ngb-muted">Agency Billing setup & controls</div>
      </template>
    ...
    <div class="min-h-0 flex-1 overflow-auto p-6" data-testid="agency-billing-accounting-policy-scroll-region">
      <div
        v-if="error"
      ...
      <template v-else>
        <div
          v-if="!hasPolicy"
          data-testid="agency-billing-accounting-policy-empty-state"
        ...
        >
          ...
          <div class="mt-1 text-sm text-ngb-muted">
            Agency Billing Accounting Policy is created by the setup workflow. Apply defaults to create the policy,
        ...
        <div v-else class="space-y-4">
          <div v-if="uiForm" data-testid="agency-billing-accounting-policy-form">
