...
const nodes = computed<SiteNavNode[]>(() => {
  return (menu.groups ?? [])
    ...
    .map((g) => {
      const id = groupId(g.label)
      const items = g.items
        .slice()
        .sort((a, b) => a.ordinal - b.ordinal)

      const rootLeaf = items.length === 1 && items[0]?.label === g.label
...
const selectedId = computed<string | null>(() => {
  ...
  for (const g of nodes.value) {
    if (routeMatches(g.route, path)) return g.id
    for (const c of g.children ?? []) {
      if (routeMatches(c.route, path)) return c.id
    }
  }
  return null
})
