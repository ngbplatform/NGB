...
vi.mock('../../src/App.vue', () => ({ default: { name: 'CRMAppStub' } }))
vi.mock('../../src/router/router', () => ({ router: mocks.router }))
vi.mock('../../src/command-palette/config', () => ({ createCRMCommandPaletteConfig: () => mocks.palette }))
vi.mock('../../src/metadata/framework', () => ({ createCRMMetadataConfig: () => mocks.metadata }))
vi.mock('../../src/editor/framework', () => ({ createCRMEditorConfig: () => mocks.editor }))

describe('CRM application bootstrap', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.window = { location: { pathname: '/documents/crm.quote', search: '?status=draft' } } as typeof window
  })
  ...
  it('redirects unauthenticated users to login before loading the app', async () => {
    ...
    await import('../../src/main')
    await vi.waitFor(() => expect(mocks.auth.login).toHaveBeenCalledWith('/documents/crm.quote?status=draft'))
