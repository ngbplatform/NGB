...
const CRM_DOCUMENT_AMOUNT_FIELD_KEYS = ['line_amount', 'amount'] as const

export type CRMDocumentPartErrors = Record<string, Record<number, Record<string, string>>>

const localRowKeys = new WeakMap<RecordPartRow, string>()

function buildCRMDocumentPartRowKey(): string {
...
export function listCRMDocumentPartFields(part: PartMetadata): FieldMetadata[] {
  return (part.list?.columns ?? [])
    .filter((column) => column.key !== 'ordinal')
    .map(createPartField)
}

export function ensureCRMDocumentPartRowKey(row: RecordPartRow): string {
  ...
  const created = buildCRMDocumentPartRowKey()
  localRowKeys.set(row, created)
  return created
}

export function normalizeCRMDocumentPartRows(rows: readonly RecordPartRow[] | null | undefined): RecordPartRow[] {
  return (rows ?? []).map((row, index) => ({
    ...row,
    __row_key: ensureCRMDocumentPartRowKey(row),
    ordinal: index + 1,
  }))
}
...
export function resolveCRMDocumentAmountSourceField(part: PartMetadata): string | null {
  const keys = new Set((part.list?.columns ?? []).map((column) => column.key))
  return CRM_DOCUMENT_AMOUNT_FIELD_KEYS.find((key) => keys.has(key)) ?? null
}

export function calculateCRMDocumentPartAmount(
  part: PartMetadata,
  rows: readonly RecordPartRow[] | null | undefined,
): number | null {
  const amountField = resolveCRMDocumentAmountSourceField(part)
  if (!amountField) return null

  let total = 0
  for (const row of normalizeCRMDocumentPartRows(rows)) {
...
export function calculateCRMDocumentAmount(
  ...
  for (const part of partsMeta) {
    const partAmount = calculateCRMDocumentPartAmount(part, partsModel?.[part.partCode]?.rows)
...
export function syncCRMDocumentAmountField(args: {
  partsMeta: readonly PartMetadata[] | null | undefined
  partsModel: RecordParts | null | undefined
  ...
  const amount = calculateCRMDocumentAmount(args.partsMeta, args.partsModel)
  if (amount == null) return

  args.model.amount = amount
}

export function buildCRMDocumentPartsPayload(
  partsMeta: readonly PartMetadata[] | null | undefined,
  partsModel: RecordParts | null | undefined,
): RecordParts | null {
  ...
  for (const part of partsMeta) {
    const fields = listCRMDocumentPartFields(part)
    const form = createPartForm(fields)
    const rows = normalizeCRMDocumentPartRows(partsModel?.[part.partCode]?.rows)
...
export async function hydrateCRMDocumentPartLookupRows(args: {
