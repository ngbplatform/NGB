...
const props = withDefaults(defineProps<{
  ...
  errors?: CRMDocumentPartErrors | null
}>(), {
  modelValue: null,
  documentModel: null,
  readonly: false,
...
function createEmptyRow(partCode: string): RecordPartRow {
  ...
  row.__row_key = ensureCRMDocumentPartRowKey(row)
...
function resolveLineAmountField(documentType: string): { quantityField: string; amountField: string } | null {
  return documentType === CRM_QUOTE
    ? { quantityField: 'quantity', amountField: 'unit_price' }
    : null
}
...
function recomputeDerivedFields(documentType: string, row: RecordPartRow): RecordPartRow {
  ...
  const discountPercent = parseDecimal(row.discount_percent) ?? 0
  const boundedDiscount = Math.min(Math.max(discountPercent, 0), 100)

  return {
    ...row,
    line_amount: roundTo4(quantity * amount * (1 - boundedDiscount / 100)),
  }
}
...
function partAmount(part: PartMetadata): number | null {
  return calculateCRMDocumentPartAmount(part, props.modelValue?.[part.partCode]?.rows)
}
...
<template>
  ...
  >
    ...
    <div
      v-if="resolveCRMDocumentAmountSourceField(part)"
