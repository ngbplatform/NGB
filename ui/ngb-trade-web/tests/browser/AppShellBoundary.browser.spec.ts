...
vi.mock('@ngbplatform/ui', async () => {
  ...
  return {
    NgbCommandPaletteDialog: {
      ...
      render: () => renderNode('div', { 'data-testid': 'trade-palette-dialog' }),
    },
    NgbSiteShell: {
      name: 'NgbSiteShell',
      ...
      setup(
        ...
        return () => renderNode('div', { 'data-testid': 'trade-shell' }, [
          renderNode('div', { 'data-testid': 'trade-shell-title' }, props.moduleTitle ?? ''),
          renderNode('div', { 'data-testid': 'trade-shell-selected-id' }, props.selectedId ?? ''),
          renderNode('div', { 'data-testid': 'trade-shell-email' }, props.userEmail ?? ''),
          renderNode('div', { 'data-testid': 'trade-shell-meta' }, props.userMeta ?? ''),
          renderNode('div', { 'data-testid': 'trade-shell-meta-icon' }, props.userMetaIcon ?? ''),
          renderNode('nav', { 'data-testid': 'trade-shell-nav' }, (props.nodes ?? []).flatMap((node) => [
            renderNode('div', { 'data-testid': 'trade-shell-node' }, node.label),
            ...(node.children ?? []).map((child) => renderNode('div', { 'data-testid': 'trade-shell-node' }, child.label)),
          ])),
          ...
          renderNode('button', { type: 'button', onClick: () => emit('navigate', 'https://status.example/trade') }, 'Navigate external'),
...
function createAuthStore(overrides: Partial<AuthStore> = {}): AuthStore {
  return reactive({
    ...
    redirecting: false,
    authenticated: false,
    userName: 'Trade Tester',
    email: 'trade@example.com',
    primaryRoleLabel: 'Trade Administrator',
  ...
  return await render(App, {
    global: {
      stubs: {
        RouterView: {
          name: 'RouterView',
          render: () => h('div', { 'data-testid': 'trade-router-view' }),
        },
      },
    },
  })
...
test('hydrates a complete shell and handles navigation, palette, and logout actions', async () => {
  ...
  await expect.element(view.getByTestId('trade-shell-title')).toHaveTextContent('Trade')
  await expect.element(view.getByTestId('trade-shell-selected-id')).toHaveTextContent('group:menu')
  await expect.element(view.getByTestId('trade-shell-email')).toHaveTextContent('')
  await expect.element(view.getByTestId('trade-shell-meta-icon')).toHaveTextContent('')
  expect(Array.from(document.querySelectorAll('[data-testid="trade-shell-node"]'), (element) => element.textContent)).toEqual([
  ...
  expect(open).toHaveBeenCalledWith('https://status.example/trade', '_self')
  ...
  expect(view.getByTestId('trade-palette-dialog').element()).not.toBeNull()
  expect(view.getByTestId('trade-router-view').element()).not.toBeNull()
})
...
test('selects an exact child route and handles an empty current path', async () => {
  ...
  const view = await renderApp()
  await expect.element(view.getByTestId('trade-shell-selected-id')).toHaveTextContent('')
  mocks.route.path = '/second'
  mocks.route.fullPath = '/second'
  await expect.element(view.getByTestId('trade-shell-selected-id')).toHaveTextContent('catalog:second')
...
test('renders empty navigation for an absent group collection', async () => {
  ...
  await expect.element(view.getByTestId('trade-shell-nav')).toHaveTextContent('')
  expect(document.querySelectorAll('[data-testid="trade-shell-node"]')).toHaveLength(0)
})
...
test('renders a bare route without shell chrome and palette', async () => {
  ...
  expect(view.getByTestId('trade-router-view').element()).not.toBeNull()
  expect(document.querySelector('[data-testid="trade-shell"]')).toBeNull()
  expect(document.querySelector('[data-testid="trade-palette-dialog"]')).toBeNull()
})
...
test('rehydrates menu and palette after logout and login', async () => {
  ...
  expect(view.getByTestId('trade-shell').element()).not.toBeNull()
})
