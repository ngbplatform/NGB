...
vi.mock('../../../src/editor/TradeEntityEditor.vue', async () => {
  ...
  return {
    default: defineComponent({
      template: '<div data-testid="trade-entity-editor-stub"></div>',
    }),
  }
})
...
describe('trade router browser integration', () => {
  beforeEach(async () => {
  ...
  it('renders the accounting policy singleton page', async () => {
    const screen = await renderAt('/catalogs/trd.accounting_policy')
  ...
  it('redirects accounting policy create route back to the singleton page', async () => {
    const screen = await renderAt('/catalogs/trd.accounting_policy/new')

    await expect.element(screen.getByTestId('accounting-policy-page')).toBeInTheDocument()
    expect(router.currentRoute.value.fullPath).toBe('/catalogs/trd.accounting_policy')
  })

  it('redirects accounting policy id route back to the singleton page', async () => {
    const screen = await renderAt('/catalogs/trd.accounting_policy/policy-1')
  ...
  it('renders metadata-driven document list routes through the shared shell', async () => {
    const screen = await renderAt('/documents/trd.sales_invoice')
  ...
  it('renders report routes through the report page component', async () => {
    const screen = await renderAt('/reports/trd.sales_by_item')
