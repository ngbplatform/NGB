...
public sealed class ReportingInternalHelperFullCoverageTests
{
    ...
    [InlineData(1, 2, null)]
    public void AccountCardCursor_Encode_OmitsIncompleteTotals(int? debit, int? credit, int? closing)
    {
        var decoded = AccountCardCursorCodec.Decode(AccountCardCursorCodec.Encode(Cursor(debit, credit, closing)));
    ...
    [InlineData(" ")]
    [InlineData("bad")]
    public void AccountCardCursor_Decode_MalformedToken_Throws(string? value)
    {
    ...
    [Fact]
    public void AccountCardCursor_RoundTripsWithAndWithoutTotals()
    {
        var withoutTotals = Cursor(totalDebit: null, totalCredit: null, closing: null);
        var current = Cursor(totalDebit: 11m, totalCredit: 12m, closing: 13m);

        var decodedWithoutTotals = AccountCardCursorCodec.Decode(AccountCardCursorCodec.Encode(withoutTotals));
        var decodedCurrent = AccountCardCursorCodec.Decode(AccountCardCursorCodec.Encode(current));

        decodedWithoutTotals.Should().BeEquivalentTo(withoutTotals);
        decodedWithoutTotals.AfterPeriodUtc.Kind.Should().Be(DateTimeKind.Utc);
