...
public sealed class BackgroundJobsHealthReporter_HealthyConfiguration_P0Tests(HangfirePostgresFixture fixture)
{
    [Fact]
    public async Task GetReportAsync_WhenCronAndTimeZoneMatch_IsNotMisconfigured()
    {
        var manager = CreateRecurringJobManager(fixture.JobStorage);
        ClearAllRecurringJobs(manager, fixture.JobStorage);

        // Register exactly what the platform would register.
    ...
    private sealed class DictionaryScheduleProvider(Dictionary<string, JobSchedule?> schedules) : IJobScheduleProvider
    {
        public JobSchedule? GetSchedule(string jobId)
            => schedules.TryGetValue(jobId, out var s) ? s : null;
    }
}
