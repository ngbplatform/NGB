...
export function accountingEffectsFlow(context: NgbScenarioContext, documentType: string, documentId?: string | null): void {
  const id = documentId ?? resolveFirstDocumentId(context, documentType);
  if (!id) {
    return;
  }

  const response = context.documents.getAccountingEffects(documentType, id);
