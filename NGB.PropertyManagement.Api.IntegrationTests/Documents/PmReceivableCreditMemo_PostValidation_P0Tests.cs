...
public sealed class PmReceivableCreditMemo_PostValidation_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivableCreditMemo_PostValidation_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    [Fact]
    public async Task PostAsync_WhenChargeTypeWasDeletedAfterDraftCreation_Returns400()
    {
        ...
        try
        {
            ...
            await catalogs.MarkForDeletionAsync(PropertyManagementCodes.ReceivableChargeType, seeded.ChargeType.Id, CancellationToken.None);

            using var resp = await client.PostAsync($"/api/documents/{PropertyManagementCodes.ReceivableCreditMemo}/{seeded.CreditMemo.Id}/post", null);
    ...
    [Fact]
    public async Task PostAsync_WhenLeaseWasDeletedAfterDraftCreation_Returns400()
    {
        var factory = new PmApiFactory(_fixture);
        try
        {
            ...
            await documents.MarkForDeletionAsync(PropertyManagementCodes.Lease, seeded.Lease.Id, CancellationToken.None);

            using var resp = await client.PostAsync($"/api/documents/{PropertyManagementCodes.ReceivableCreditMemo}/{seeded.CreditMemo.Id}/post", null);
            resp.StatusCode.Should().Be(HttpStatusCode.BadRequest);

            using var doc = JsonDocument.Parse(await resp.Content.ReadAsStringAsync());
            doc.RootElement.GetProperty("error").GetProperty("code").GetString().Should().Be("pm.validation.receivable_credit_memo.lease_deleted");
        }
        finally
        {
            await DisposeFactoryAsync(factory);
    ...
    private static async Task<(DocumentDto Lease, CatalogItemDto ChargeType, DocumentDto CreditMemo)> SeedDraftCreditMemoAsync(IServiceProvider services)
    {
        ...
        var creditMemo = await documents.CreateDraftAsync(PropertyManagementCodes.ReceivableCreditMemo, Payload(new { lease_id = lease.Id, charge_type_id = chargeType.Id, credited_on_utc = "2026-02-08", amount = "25.00" }), CancellationToken.None);
        return (lease, chargeType, creditMemo);
    }
