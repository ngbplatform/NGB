...
public sealed class PmReceivableReturnedPayment_LeaseConsistency_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;
    private static readonly JsonSerializerOptions Json = CreateJson();

    public PmReceivableReturnedPayment_LeaseConsistency_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    [Fact]
    public async Task Metadata_Uses_OriginalPaymentOnly_Context_Fields()
    {
        using var factory = new PmApiFactory(_fixture);
        using var client = factory.CreateClient(new WebApplicationFactoryClientOptions { BaseAddress = new Uri("https://localhost") });

        using var resp = await client.GetAsync($"/api/documents/{PropertyManagementCodes.ReceivableReturnedPayment}/metadata");
        ...
        fields.Should().Contain(f => f.Key == "original_payment_id");
        fields.Should().NotContain(f => f.Key == "party_id");
        fields.Should().NotContain(f => f.Key == "property_id");
        fields.Should().NotContain(f => f.Key == "lease_id");
        fields.Should().NotContain(f => f.Key == "bank_account_id");
    }

    [Fact]
    public async Task CreateDraft_WithOriginalPaymentOnlyContext_Succeeds()
    {
        ...
        try
        {
            ...
            returned.Status.Should().Be(DocumentStatus.Draft);
        }
        finally
        {
