...
public sealed class PmReceivablePayment_VariantB_History_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivablePayment_VariantB_History_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task PostUnpostPost_KeepsAppendOnlyLifecycleAndSubsystemHistory()
    {
        ...
        try
        {
            ...
            (await documents.PostAsync(PropertyManagementCodes.ReceivablePayment, seeded.ReceivablePayment.Id, CancellationToken.None))
                .Status.Should().Be(DocumentStatus.Posted);

            (await documents.UnpostAsync(PropertyManagementCodes.ReceivablePayment, seeded.ReceivablePayment.Id, CancellationToken.None))
                .Status.Should().Be(DocumentStatus.Draft);

            (await documents.PostAsync(PropertyManagementCodes.ReceivablePayment, seeded.ReceivablePayment.Id, CancellationToken.None))
                .Status.Should().Be(DocumentStatus.Posted);

            var current = await documents.GetByIdAsync(PropertyManagementCodes.ReceivablePayment, seeded.ReceivablePayment.Id, CancellationToken.None);
            ...
            var documentHistory = await uow.Connection.QuerySingleAsync<int>(
                new CommandDefinition(
                    ...
                    new { document_id = seeded.ReceivablePayment.Id },
...
""";
            ...
            var accountingRows = (await uow.Connection.QueryAsync<AccountingEntryRow>(
                new CommandDefinition(accountingSql, new { document_id = seeded.ReceivablePayment.Id }, uow.Transaction, cancellationToken: CancellationToken.None)))
                .AsList();
            ...
            var movements = await opregRead.GetMovementsPageAsync(
                new OperationalRegisterMovementsPageRequest(
                    ...
                    DocumentId: seeded.ReceivablePayment.Id,
                    PageSize: 50),
    ...
    private static async Task<(DocumentDto ReceivablePayment, Guid RegisterId)> SeedDraftReceivablePaymentAsync(IServiceProvider services)
    {
        ...
        var receivablePayment = await documents.CreateDraftAsync(PropertyManagementCodes.ReceivablePayment, Payload(new
        {
            lease_id = lease.Id,
        ...
        return (receivablePayment, setupResult.ReceivablesOpenItemsOperationalRegisterId);
    }
