...
public sealed class PmReceivableCreditMemo_LeaseConsistency_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;
    private static readonly JsonSerializerOptions Json = CreateJson();

    public PmReceivableCreditMemo_LeaseConsistency_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    [Fact]
    public async Task Metadata_Uses_LeaseAndChargeType_Only()
    {
        using var factory = new PmApiFactory(_fixture);
        using var client = factory.CreateClient(new WebApplicationFactoryClientOptions { BaseAddress = new Uri("https://localhost") });

        using var resp = await client.GetAsync($"/api/documents/{PropertyManagementCodes.ReceivableCreditMemo}/metadata");
    ...
    [Fact]
    public async Task CreateDraft_WithLeaseAndChargeType_Succeeds()
    {
        ...
        try
        {
            ...
            var memo = await documents.CreateDraftAsync(PropertyManagementCodes.ReceivableCreditMemo, Payload(new
            {
                ...
                amount = "25.00",
                memo = "Standalone credit"
            }), CancellationToken.None);

            memo.Status.Should().Be(DocumentStatus.Draft);
        }
        finally
        {
