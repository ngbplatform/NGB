...
public sealed class PmReceivablePayment_Unpost_StaleCompletedLog_Conflicts_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivablePayment_Unpost_StaleCompletedLog_Conflicts_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task UnpostAsync_WhenCompletedUnpostLogExistsButDocumentIsStillPosted_FailsFast_AndStatusRemainsPosted()
    {
        ...
        try
        {
            ...
            var receivablePayment = await SeedDraftReceivablePaymentAsync(scope.ServiceProvider);
            ...
            {
                document_id = receivablePayment.Id,
                operation = 2,
            ...
            var act = () => documents.UnpostAsync(PropertyManagementCodes.ReceivablePayment, receivablePayment.Id, CancellationToken.None);
            await act.Should().ThrowAsync<NgbInvariantViolationException>()
                .WithMessage("*inconsistent*");

            var current = await documents.GetByIdAsync(PropertyManagementCodes.ReceivablePayment, receivablePayment.Id, CancellationToken.None);
    ...
    private static async Task<DocumentDto> SeedDraftReceivablePaymentAsync(IServiceProvider services)
    {
        ...
        return await documents.CreateDraftAsync(PropertyManagementCodes.ReceivablePayment, Payload(new
        {
            lease_id = lease.Id,
