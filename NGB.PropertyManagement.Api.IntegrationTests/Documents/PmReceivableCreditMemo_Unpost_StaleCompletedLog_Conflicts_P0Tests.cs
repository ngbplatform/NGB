...
public sealed class PmReceivableCreditMemo_Unpost_StaleCompletedLog_Conflicts_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivableCreditMemo_Unpost_StaleCompletedLog_Conflicts_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task UnpostAsync_WhenCompletedUnpostLogExistsButDocumentIsStillPosted_FailsFast_AndStatusRemainsPosted()
    {
        ...
        try
        {
            ...
            var creditMemo = await SeedDraftCreditMemoAsync(scope.ServiceProvider);
            (await documents.PostAsync(PropertyManagementCodes.ReceivableCreditMemo, creditMemo.Id, CancellationToken.None)).Status.Should().Be(DocumentStatus.Posted);

            await EnsureCompletedLogAsync(uow, "accounting_posting_state", new
            {
                document_id = creditMemo.Id,
                operation = 2,
            ...
            var act = () => documents.UnpostAsync(PropertyManagementCodes.ReceivableCreditMemo, creditMemo.Id, CancellationToken.None);
            await act.Should().ThrowAsync<NgbInvariantViolationException>()
                .WithMessage("*inconsistent*");

            var current = await documents.GetByIdAsync(PropertyManagementCodes.ReceivableCreditMemo, creditMemo.Id, CancellationToken.None);
    ...
    private static async Task<DocumentDto> SeedDraftCreditMemoAsync(IServiceProvider services)
    {
        ...
        return await documents.CreateDraftAsync(PropertyManagementCodes.ReceivableCreditMemo, Payload(new
            {
            lease_id = lease.Id,
            charge_type_id = chargeType.Id,
