...
public sealed class PmReceivablePayment_Validation_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;
    private static readonly JsonSerializerOptions Json = CreateJson();

    public PmReceivablePayment_Validation_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task Create_WhenAmountIsNotPositive_Returns400_WithFriendlyFieldError()
    {
        ...
        try
        {
            ...
            using var resp = await client.PostAsJsonAsync($"/api/documents/{PropertyManagementCodes.ReceivablePayment}", new
            {
                fields = new
                {
                    lease_id = lease.Id,
            ...
            doc.RootElement.GetProperty("error").GetProperty("code").GetString().Should().Be("pm.validation.receivable_payment.amount_must_be_positive");
        }
        finally
        {
    ...
    public async Task Metadata_DoesNotExpose_Party_Or_Property_Fields()
    {
        ...
        using var metaResp = await client.GetAsync($"/api/documents/{PropertyManagementCodes.ReceivablePayment}/metadata");
    ...
    private static async Task<(CatalogItemDto Party, CatalogItemDto Building, CatalogItemDto Unit, DocumentDto Lease)> CreateLeaseAsync(ICatalogService catalogs, IDocumentService documents)
    {
