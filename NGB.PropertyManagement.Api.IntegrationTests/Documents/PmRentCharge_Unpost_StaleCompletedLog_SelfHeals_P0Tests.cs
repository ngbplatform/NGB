...
public sealed class PmRentCharge_Unpost_StaleCompletedLog_Conflicts_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmRentCharge_Unpost_StaleCompletedLog_Conflicts_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task UnpostAsync_WhenCompletedUnpostLogExistsButDocumentIsStillPosted_FailsFast_AndStatusRemainsPosted()
    {
        ...
        try
        {
            ...
            var rentCharge = await SeedDraftRentChargeAsync(scope.ServiceProvider);
            (await documents.PostAsync(PropertyManagementCodes.RentCharge, rentCharge.Id, CancellationToken.None))
                .Status.Should().Be(DocumentStatus.Posted);

            await EnsureCompletedLogAsync(uow, "accounting_posting_state", new
            {
                document_id = rentCharge.Id,
                operation = 2,
            ...
            var act = () => documents.UnpostAsync(PropertyManagementCodes.RentCharge, rentCharge.Id, CancellationToken.None);
            await act.Should().ThrowAsync<NgbInvariantViolationException>()
                .WithMessage("*inconsistent*");

            var current = await documents.GetByIdAsync(PropertyManagementCodes.RentCharge, rentCharge.Id, CancellationToken.None);
    ...
    private static async Task<DocumentDto> SeedDraftRentChargeAsync(IServiceProvider services)
    {
        ...
        return await documents.CreateDraftAsync(PropertyManagementCodes.RentCharge, Payload(new
        {
            lease_id = lease.Id,
