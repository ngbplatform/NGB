...
public sealed class PmReceivableCharge_Unpost_StaleCompletedLog_Conflicts_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivableCharge_Unpost_StaleCompletedLog_Conflicts_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task UnpostAsync_WhenCompletedUnpostLogExistsButDocumentIsStillPosted_FailsFast_AndStatusRemainsPosted()
    {
        ...
        try
        {
            ...
            var receivableCharge = await SeedDraftReceivableChargeAsync(scope.ServiceProvider);
            (await documents.PostAsync(PropertyManagementCodes.ReceivableCharge, receivableCharge.Id, CancellationToken.None))
                .Status.Should().Be(DocumentStatus.Posted);

            await EnsureCompletedLogAsync(uow, "accounting_posting_state", new
            {
                document_id = receivableCharge.Id,
                operation = 2,
            ...
            var act = () => documents.UnpostAsync(PropertyManagementCodes.ReceivableCharge, receivableCharge.Id, CancellationToken.None);
            await act.Should().ThrowAsync<NgbInvariantViolationException>()
                .WithMessage("*inconsistent*");

            var current = await documents.GetByIdAsync(PropertyManagementCodes.ReceivableCharge, receivableCharge.Id, CancellationToken.None);
    ...
    private static async Task<DocumentDto> SeedDraftReceivableChargeAsync(IServiceProvider services)
    {
        ...
        return await documents.CreateDraftAsync(PropertyManagementCodes.ReceivableCharge, Payload(new
        {
            lease_id = lease.Id,
