...
public sealed class PmReceivableReturnedPayment_VariantB_History_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivableReturnedPayment_VariantB_History_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;
    ...
    public async Task PostUnpostPost_KeepsAppendOnlyLifecycleAndSubsystemHistory()
    {
        ...
        try
        {
            ...
            var current = await documents.GetByIdAsync(PropertyManagementCodes.ReceivableReturnedPayment, seeded.ReturnedPayment.Id, CancellationToken.None);
            ...
            var documentHistory = await uow.Connection.QuerySingleAsync<int>(
                new CommandDefinition(
                    ...
                    new { document_id = seeded.ReturnedPayment.Id },
...
""";
            ...
            var accountingRows = (await uow.Connection.QueryAsync<AccountingEntryRow>(
                new CommandDefinition(accountingSql, new { document_id = seeded.ReturnedPayment.Id }, uow.Transaction, cancellationToken: CancellationToken.None)))
                .AsList();
    ...
    private static async Task<(DocumentDto ReturnedPayment, Guid RegisterId)> SeedDraftReturnedPaymentAsync(IServiceProvider services)
    {
