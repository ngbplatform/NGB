...
public sealed class PmReceivableCharge_PartyRoleValidation_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;
    private static readonly JsonSerializerOptions Json = CreateJson();

    public PmReceivableCharge_PartyRoleValidation_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    [Fact]
    public async Task Metadata_DoesNotExposePartyField()
    {
    ...
    [Fact]
    public async Task Post_WhenLeasePrimaryPartyLostTenantRoleAfterDraft_Returns400_WithFriendlyFieldError()
    {
        ...
        try
        {
            ...
            var (party, _, _, lease, rentType) = await CreateLeaseAndChargeTypeAsync(catalogs, documents);
            var charge = await documents.CreateDraftAsync(PropertyManagementCodes.ReceivableCharge, Payload(new
            {
                ...
                amount = "123.45",
                memo = "Tenant receivable"
            }), CancellationToken.None);

            await catalogs.UpdateAsync(PropertyManagementCodes.Party, party.Id, Payload(new
            {
                display = "P",
                is_tenant = false,
                is_vendor = true
            ...
            doc.RootElement.GetProperty("error").GetProperty("code").GetString().Should().Be("pm.receivable_charge.party.must_be_tenant");
        }
        finally
        {
    ...
    private static async Task<(CatalogItemDto Party, CatalogItemDto Building, CatalogItemDto Unit, DocumentDto Lease, CatalogItemDto RentType)> CreateLeaseAndChargeTypeAsync(ICatalogService catalogs, IDocumentService documents)
    {
        ...
        var unit = await catalogs.CreateAsync(PropertyManagementCodes.Property, Payload(new { kind = "Unit", parent_property_id = building.Id, unit_no = "1A" }), CancellationToken.None);
