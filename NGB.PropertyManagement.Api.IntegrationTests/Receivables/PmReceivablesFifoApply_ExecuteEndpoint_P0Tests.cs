...
public sealed class PmReceivablesFifoApply_ExecuteEndpoint_P0Tests : IAsyncLifetime
{
    private readonly PmIntegrationFixture _fixture;

    public PmReceivablesFifoApply_ExecuteEndpoint_P0Tests(PmIntegrationFixture fixture) => _fixture = fixture;

    public async Task InitializeAsync() => await _fixture.ResetDatabaseAsync();
    public Task DisposeAsync() => Task.CompletedTask;

    [Fact]
    public async Task ExecuteFifoApply_CreatesAndPostsApplyDocs_AndUpdatesOpenItems()
    {
            ...
            var property = await catalogs.CreateAsync(PropertyManagementCodes.Property, Payload(new
        {
            ...
            var req = new ReceivablesFifoApplyExecuteRequest(payment.Id, MaxApplications: null);
            var http = await client.PostAsJsonAsync("/api/receivables/apply/fifo/execute", req);
            http.EnsureSuccessStatusCode();

            var resp = await http.Content.ReadFromJsonAsync<ReceivablesFifoApplyExecuteResponse>();
            ...
            open.Charges[0].ChargeDocumentId.Should().Be(c2.Id);
