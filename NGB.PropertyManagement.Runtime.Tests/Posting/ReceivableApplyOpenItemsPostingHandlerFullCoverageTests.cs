...
public sealed class ReceivableApplyOpenItemsPostingHandlerFullCoverageTests
{
    [Fact]
    ...
    public async Task Draft_locks_in_first_order_creates_relationships_and_builds_two_movements()
    {
        ...
        fixture.Sut.TypeCode.Should().Be(PropertyManagementCodes.ReceivableApply);
        ...
        fixture.Bags.Should().HaveCount(2).And.OnlyContain(x => x.Count == 4);
    }
    ...
    [Fact]
    public async Task Late_fee_and_rent_are_supported_when_regular_charge_reader_does_not_match()
    {
        var late = new Fixture();
        late.UseLateFeeCharge();
        await late.Sut.BuildMovementsAsync(late.Document(DocumentStatus.Draft), late.Builder.Object, default);
        late.Movements.Should().HaveCount(2);

        var rent = new Fixture();
        rent.UseRentCharge();
        await rent.Sut.BuildMovementsAsync(rent.Document(DocumentStatus.Draft), rent.Builder.Object, default);
        rent.Movements.Should().HaveCount(2);
    }

    [Fact]
    public async Task Credit_source_party_property_and_lease_must_match_charge()
    {
        ...
        lease.SetPayment(leaseId: Guid.CreateVersion7());
    ...
    private sealed class Fixture
    {
        public Fixture(bool reverseIds = false, bool hasRegister = true)
        {
            ...
            Documents.Setup(x => x.GetAsync(CreditId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(Document(CreditId, PropertyManagementCodes.ReceivablePayment, DocumentStatus.Posted));
            ...
            Builder.Setup(x => x.Add("pm.receivables.open_items", It.IsAny<OperationalRegisterMovement>()))
                .Callback<string, OperationalRegisterMovement>((_, movement) => Movements.Add(movement));
            Sut = new ReceivableApplyOpenItemsOperationalRegisterPostingHandler(
                Readers.Object, Documents.Object, Policy.Object, Registers.Object, Net.Object,
                DimensionSets.Object, Relationships.Object, Locks.Object);
        }
        ...
        public ReceivableApplyOpenItemsOperationalRegisterPostingHandler Sut { get; }
        ...
        public void SetPayment(Guid? partyId = null, Guid? propertyId = null, Guid? leaseId = null)
            => Readers.Setup(x => x.ReadReceivablePaymentHeadAsync(CreditId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new PmReceivablePaymentHead(
                    CreditId, partyId ?? PartyId, propertyId ?? PropertyId, leaseId ?? LeaseId,
                    null, new DateOnly(2026, 8, 1), 10m, null));

        public void UseReceivableCharge()
            => Readers.Setup(x => x.ReadReceivableChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new PmReceivableChargeHead(
                    ChargeId, PartyId, PropertyId, LeaseId, Guid.CreateVersion7(),
                    new DateOnly(2026, 8, 1), 10m, null));

        public void UseLateFeeCharge()
        {
            Readers.Setup(x => x.ReadReceivableChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ThrowsAsync(new InvalidOperationException("not receivable charge"));
            Readers.Setup(x => x.ReadLateFeeChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new PmLateFeeChargeHead(
                    ChargeId, PartyId, PropertyId, LeaseId, new DateOnly(2026, 8, 1), 10m, null));
        }

        public void UseRentCharge()
        {
            Readers.Setup(x => x.ReadReceivableChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ThrowsAsync(new InvalidOperationException("not receivable charge"));
            Readers.Setup(x => x.ReadLateFeeChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ThrowsAsync(new InvalidOperationException("not late fee"));
            Readers.Setup(x => x.ReadRentChargeHeadAsync(ChargeId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new PmRentChargeHead(
                    ChargeId, LeaseId, PartyId, PropertyId, new DateOnly(2026, 8, 1),
                    new DateOnly(2026, 8, 31), new DateOnly(2026, 8, 1), 10m, null));
        }

        public DocumentRecord Document(DocumentStatus status)
            => Document(DocumentId, PropertyManagementCodes.ReceivableApply, status);
