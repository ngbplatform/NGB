...
public sealed class PayablesApplyBatchServiceFullCoverageTests
{
    ...
    public async Task Batch_accepts_25_applications_in_one_transaction()
    {
        ...
        var result = await fixture.Sut.ExecuteAsync(new PayablesApplyBatchRequest(items));
    ...
    {
        var fixture = new Fixture();
        var request = new PayablesApplyBatchRequest(Enumerable.Repeat(fixture.Item(), 26).ToArray());

        await ((Func<Task>)(() => fixture.Sut.ExecuteAsync(request)))
            .Should().ThrowAsync<PayablesApplyBatchValidationException>();
