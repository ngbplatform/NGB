...
/// <summary>
/// P1: Cross-module atomicity contract for Document.PostAsync when multiple subsystems participate:
/// - Accounting (register + posting_log)
/// - Operational Registers (movements + write_log + finalizations)
/// - Reference Registers (independent write log + records + has_records flag)
    ...
    : IntegrationTestBase(fixture)
{
    ...
    [Fact]
    public async Task PostAsync_WhenAuditWriterThrowsAfterWrite_RollsBack_Accounting_Opreg_RefReg_PostingLog_Audit_And_Actor()
    {
        ...
        var refregCode = "it_refreg_" + Guid.CreateVersion7().ToString("N")[..8];
        var referenceRegisterId = await CreateReferenceRegisterAsync(host, refregCode);
        var refregCommandId = Guid.CreateVersion7();
        ...
        await using (var scope = host.Services.CreateAsyncScope())
        {
            ...
            var refreg = scope.ServiceProvider.GetRequiredService<IReferenceRegisterIndependentWriteService>();
            ...
            var act = () => posting.PostAsync(docId, async (ctx, ct) =>
            {
                ...
                // Reference Register independent write must participate in the SAME transaction.
                // We intentionally use DimensionSetId=Guid.Empty and NonPeriodic period to keep the setup minimal.
                await refreg.UpsertByDimensionSetIdAsync(
                    registerId: referenceRegisterId,
                    dimensionSetId: Guid.Empty,
                    periodUtc: null,
                    values: new Dictionary<string, object?>(),
                    commandId: refregCommandId,
        ...
        await AssertNoPostSideEffectsAsync(docId, registerId, referenceRegisterId, refregCommandId, baseline);
    }
    ...
    private static async Task<Guid> CreateReferenceRegisterAsync(IHost host, string code)
    {
        await using var scope = host.Services.CreateAsyncScope();
    ...
    private async Task AssertNoPostSideEffectsAsync(
        Guid documentId,
        Guid operationalRegisterId,
        Guid referenceRegisterId,
        Guid referenceRegisterCommandId,
        AuditBaseline baseline)
    {
        ...
        (await conn.ExecuteScalarAsync<int>(
            ...
            new { r = operationalRegisterId, p = period }))
        ...
        // Reference Register independent write must rollback together with document posting.
        (await conn.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM reference_register_independent_write_state WHERE register_id = @r AND command_id = @c AND operation = 1;",
            new { r = referenceRegisterId, c = referenceRegisterCommandId }))
            .Should().Be(0, "failed Post must not persist reference register independent write_log row");

        (await conn.ExecuteScalarAsync<bool>(
            "SELECT has_records FROM reference_registers WHERE register_id = @id;",
            new { id = referenceRegisterId }))
            .Should().BeFalse("failed Post must rollback has_records update");

        var refTableCode = await conn.ExecuteScalarAsync<string?>(
            "SELECT table_code FROM reference_registers WHERE register_id = @id;",
            new { id = referenceRegisterId });

        refTableCode.Should().NotBeNullOrWhiteSpace();
        var recordsTable = ReferenceRegisterNaming.RecordsTable(refTableCode!);
        var recordsTableExists = await conn.ExecuteScalarAsync<bool>(
            "SELECT to_regclass('public.' || @tbl) IS NOT NULL;",
            new { tbl = recordsTable });

        if (recordsTableExists)
        {
            (await conn.ExecuteScalarAsync<int>($"SELECT COUNT(*) FROM {recordsTable};"))
                .Should().Be(0, "failed Post must rollback reference register records");
        }
        (await conn.ExecuteScalarAsync<int>(
        ...
        : IDocumentOperationalRegisterPostingHandler
    {
            ...
            CancellationToken ct = default)
        {
            if (string.IsNullOrWhiteSpace(state.RegisterCode))
                throw new NotSupportedException("Test state RegisterCode is not configured.");
