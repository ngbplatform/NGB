...
public sealed class DocumentPostingService_PostValidators_Failure_RollsBack_NumberingAndSideEffects_P0Tests(SchemaPostgresTestFixture fixture)
    : IntegrationTestBase(fixture)
{
    private const string TypeCode = "it_doc_postval_rb";
    private const string TypedTable = "doc_it_doc_postval_rb";

    [Fact]
    public async Task PostAsync_WhenPostValidatorThrows_RollsBackNumberingTypedSyncAndPostingSideEffects()
    {
        ...
        using var host = IntegrationHostFactory.Create(
            ...
            services =>
            {
                ...
                services.AddScoped<ItThrowingPostValidator>();
            });

        await ReportingTestHelpers.SeedMinimalCoAAsync(host);

        var date = new DateTime(2026, 01, 12, 0, 0, 0, DateTimeKind.Utc);
        ...
        await using (var scope = host.Services.CreateAsyncScope())
        {
            ...
            await act.Should().ThrowAsync<NotSupportedException>()
                .WithMessage("*simulated post validator failure*");
        }

        postingActionCalled.Should().BeFalse("posting action must not run if post validation fails");
        ...
        doc.Number.Should().BeNull("number assignment on post must rollback on post validator failure");
        ...
        typed.UpdateCalls.Should().Be(0, "typed sync on number assignment must rollback together with PostAsync");
        ...
        var postAuditCount = await conn.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM platform_audit_events WHERE entity_id = @id AND action_code = @code;",
            new { id, code = AuditActionCodes.DocumentPost });
        postAuditCount.Should().Be(0, "document.post audit must not be persisted on rollback");
    }
        ...
        public void Contribute(DefinitionsBuilder builder)
        {
            builder.AddDocument(TypeCode, b => b
                .Metadata(new DocumentTypeMetadata(
                    ...
                    new DocumentPresentationMetadata("IT Doc PostValidator Rollback"),
                    new DocumentMetadataVersion(1, "it-tests")))
                .TypedStorage<ItDocStorage>()
                .NumberingPolicy<ItDocNumberingPolicy>()
                .AddPostValidator<ItThrowingPostValidator>());
        }
    }

    private sealed class ItDocNumberingPolicy : IDocumentNumberingPolicy
    {
        public string TypeCode => DocumentPostingService_PostValidators_Failure_RollsBack_NumberingAndSideEffects_P0Tests.TypeCode;
        public bool EnsureNumberOnCreateDraft => false;
        public bool EnsureNumberOnPost => true;
    }

    private sealed class ItThrowingPostValidator : IDocumentPostValidator
    {
        public string TypeCode => DocumentPostingService_PostValidators_Failure_RollsBack_NumberingAndSideEffects_P0Tests.TypeCode;

        public Task ValidateBeforePostAsync(DocumentRecord documentForUpdate, CancellationToken ct)
            => throw new NotSupportedException("simulated post validator failure");
    }
...
""";
            ...
            await uow.Connection.ExecuteAsync(new CommandDefinition(
                sql,
                new { documentId = updatedDraft.Id, number = updatedDraft.Number },
                uow.Transaction,
