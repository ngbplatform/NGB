...
public sealed class PostingRecoveryAfterCancellationTests(PostgresTestFixture fixture)
{
    ...
    [Fact(Timeout = 30_000)]
    public async Task PostAsync_WhenFirstAttemptCancelledInsideEntryWrite_DoesNotPoisonSubsequentPosting()
    {
        ...
        using var host = IntegrationHostFactory.Create(
            ...
            {
                // Replace the entry writer with: "cancel-first-write, then delegate to real postgres writer".
        ...
        try
        {
            // 1) First attempt: cancel while inside entry-writer (via real CancellationToken), ensure rollback.
            postTask = PostOnceAsync(host, failedDocId, period, cts.Token);

            // Wait until the writer is actually inside its "waiting-for-cancellation" section.
            await probe.EntryWriterBlocking.Task.WaitAsync(TimeSpan.FromSeconds(10));
    ...
    private sealed class CancellationProbe
    {
        ...
        public TaskCompletionSource EntryWriterBlocking { get; } =
            new(TaskCreationOptions.RunContinuationsAsynchronously);
    }

    private sealed class CancelFirstEntryWriter(CancellationProbe probe, IAccountingEntryWriter inner) : IAccountingEntryWriter
    {
        public async Task WriteAsync(IReadOnlyList<AccountingEntry> entries, CancellationToken ct = default)
        {
            ...
            if (call == 1)
            {
                // Signal that we're inside the entry-writer. Cancellation must occur now.
                probe.EntryWriterBlocking.TrySetResult();

                // Wait for cancellation, but never hang the test run:
                // - if cancellation doesn't arrive quickly -> fail fast (instead of holding locks/txns).
                var cancelWait = Task.Delay(Timeout.InfiniteTimeSpan, ct);
                var guard = Task.Delay(TimeSpan.FromSeconds(5));

                var completed = await Task.WhenAny(cancelWait, guard);
                if (completed == guard)
                {
                    throw new NotSupportedException(
                        $"Expected cancellation to occur inside {nameof(IAccountingEntryWriter)}.{nameof(WriteAsync)} on the first call, " +
                        $"but it did not. TokenCanBeCanceled={ct.CanBeCanceled}. This would risk poisoning the test run.");
                }

                // Will throw OperationCanceledException (expected).
                await cancelWait;
            }
            else
            {
