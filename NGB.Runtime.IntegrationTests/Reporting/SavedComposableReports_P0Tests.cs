...
public sealed class SavedComposableReports_P0Tests(PostgresTestFixture fixture) : IntegrationTestBase(fixture)
{
    [Theory]
    [InlineData("flat")]
    [InlineData("grouped")]
    ...
    public async Task Complete_large_reports_preserve_global_averages_pages_and_exports(string shape)
    {
        ...
        using var host = IntegrationHostFactory.Create(Fixture.ConnectionString, services =>
        {
            ...
            services.AddSingleton<ILogger<ReportRunProcessor>>(log);
        });
        ...
        var run = await runs.StartAsync(Source.Code, "reader", request, default);
        await host.Services.GetRequiredService<ReportRunProcessor>().ProcessNextAsync(default);
        (await runs.GetAsync(Source.Code, "reader", run.Id, default))!.Status.Should().Be("Ready",
            string.Join("\n", log.Errors));
        var rows = new List<ReportSheetRowDto>();
        ReportSheetDto? template = null;
        for (var offset = 0;;)
        {
            var page = await runs.ReadAsync(Source.Code, "reader", run.Id, offset, 271, default);
            template = page.Sheet;
            rows.AddRange(page.Sheet.Rows);
            if (!page.HasMore) break;
            offset += page.Sheet.Rows.Count;
        }
        rows.Count(r => r.RowKind == ReportRowKind.Detail).Should().Be(12001);
        var total = rows.Single(r => r.RowKind == ReportRowKind.Total);
        total.Cells.Last().Value!.Value.GetDecimal().Should().Be(6002m, "global averages must use original observations, not subgroup averages");
        if (shape != "flat") rows.Count(r => r.RowKind == ReportRowKind.Group).Should().Be(4);
        await using var file = await runs.ExportAsync(Source.Code, "reader", run.Id, default);
        ...
        var workbookRows = XDocument.Load(xml).Descendants(XName.Get("row", "http://schemas.openxmlformats.org/spreadsheetml/2006/main")).Count();
        workbookRows.Should().Be(rows.Count + (template!.HeaderRows?.Count ?? 1));
    }

    internal sealed class CapturedLog : ILogger<ReportRunProcessor>
    {
    ...
    public async Task Subtotals_and_pivot_totals_use_original_observations_and_preserve_measure_sorting(bool pivot, ReportAggregationKind aggregation, decimal expected)
    {
        ...
        using var host = IntegrationHostFactory.Create(Fixture.ConnectionString, services =>
        { services.AddSingleton<IReportDefinitionSource>(source); services.AddSingleton<IPostgresReportDatasetSource>(source); services.AddSingleton<ILogger<ReportRunProcessor>, CapturedLog>(); });
        var sheet = await SavedAccountingReports_P0Tests.RunAsync(host, Source.Code, new(Layout: new(
    ...
    public async Task A_measure_only_report_preserves_its_first_numeric_result()
    {
        ...
        using var host = IntegrationHostFactory.Create(Fixture.ConnectionString, services =>
        { services.AddSingleton<IReportDefinitionSource>(source); services.AddSingleton<IPostgresReportDatasetSource>(source); services.AddSingleton<ILogger<ReportRunProcessor>, CapturedLog>(); });
        var sheet = await SavedAccountingReports_P0Tests.RunAsync(host, Source.Code, new(Layout: new(Measures: [new("amount", ReportAggregationKind.Average)], ShowGrandTotals: true)));
    ...
    public async Task Source_pages_and_separate_total_queries_share_a_repeatable_read_snapshot()
    {
        ...
        await SavedAccountingReports_P0Tests.ExecuteAsync(host,
            "CREATE TABLE it_saved_rows(x integer); INSERT INTO it_saved_rows SELECT generate_series(1,1001);", null);
        await using var scope = host.Services.CreateAsyncScope();
        var runs = scope.ServiceProvider.GetRequiredService<IReportRunService>();
        var run = await runs.StartAsync(Source.Code, "reader", new(Layout: new(DetailFields: ["id"],
            Measures: [new("amount", ReportAggregationKind.Average)], ShowGrandTotals: true)), default);
        var processing = host.Services.GetRequiredService<ReportRunProcessor>().ProcessNextAsync(default);
        try
        {
            await gate.Read.Task.WaitAsync(TimeSpan.FromSeconds(15));
            await SavedAccountingReports_P0Tests.ExecuteAsync(host, "INSERT INTO it_saved_rows VALUES (1000000);", null);
        }
        finally { gate.Resume.TrySetResult(); }
        await processing;
        var page = await runs.ReadAsync(Source.Code, "reader", run.Id, 1000, 10, default);
        page.Total.Should().Be(1002);
        page.Sheet.Rows.Last().Cells.Last().Value!.Value.GetDecimal().Should().Be(502m);
        await SavedAccountingReports_P0Tests.ExecuteAsync(host, "DROP TABLE it_saved_rows;", null);
    }
    ...
    private sealed class Source(string fromSql = "generate_series(1,12001) x CROSS JOIN generate_series(1,2) y") : IReportDefinitionSource, IPostgresReportDatasetSource
    {
