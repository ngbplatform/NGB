...
public sealed class DirectAccountingExportsTests(PostgresTestFixture fixture) : IntegrationTestBase(fixture)
{
    ...
    public async Task Integrity_checks_cover_all_dimension_keys_and_export_merged_totals(bool closed)
    {
        ...
        var viewed = await ReadPagesAsync(host, "accounting.consistency", new(Parameters: new Dictionary<string, string> { ["period_utc"] = "2026-09-01" }));
        viewed.Select(Signature).Should().Equal(result.Rows.Select(Signature));
    ...
    public async Task Canonical_journals_continue_beyond_ten_thousand_rows_without_losing_running_balances(string code)
    {
        ...
        var viewed = await ReadPagesAsync(host, code, request);
        viewed.Select(Signature).Should().Equal(sheet.Rows.Select(Signature));
    ...
    private static string Signature(ReportSheetRowDto row) => string.Join('|', row.Cells.Select(c => c.Display));

    private static async Task<IReadOnlyList<ReportSheetRowDto>> ReadPagesAsync(IHost host, string code, ReportExecutionRequestDto request)
    {
        await using var scope = host.Services.CreateAsyncScope();
        ...
        do
        {
            page = await engine.ExecuteAsync(code, request with { Cursor = cursor, Limit = 233 }, default);
            page.Sheet.Rows.Count.Should().BeLessThan(510);
            if (page.HasMore) page.Sheet.Rows.Should().NotContain(r => r.RowKind == ReportRowKind.Total);
            rows.AddRange(page.Sheet.Rows);
            if (page.HasMore) page.NextCursor.Should().NotBeNullOrEmpty().And.NotBe(cursor, "each non-final page must advance");
    ...
    {
        await using var scope = host.Services.CreateAsyncScope();
        var capture = new CapturingExporter();
        var service = ActivatorUtilities.CreateInstance<ReportDownloadService>(scope.ServiceProvider, capture);
        using var file = new MemoryStream();
        await using (var download = await service.PrepareAsync(code,
            new(request.Layout, request.Filters, request.Parameters, request.VariantCode), default))
            await download.WriteAsync(file, default);
        file.Position = 0;
        using var zip = new ZipArchive(file, ZipArchiveMode.Read);
        using var xml = zip.GetEntry("xl/worksheets/sheet1.xml")!.Open();
        var worksheet = XDocument.Load(xml);
        XNamespace ns = "http://schemas.openxmlformats.org/spreadsheetml/2006/main";
        worksheet.Descendants(ns + "row").Count().Should().Be(capture.Sheet!.Rows.Count + (capture.Sheet.HeaderRows?.Count ?? 1));
        if (code == "accounting.consistency") worksheet.Descendants(ns + "mergeCell").Should().HaveCount(5);
        return capture.Sheet;
    }

    private sealed class CapturingExporter : IStreamingReportExportService
    {
        public ReportSheetDto? Sheet { get; private set; }
        public async Task WriteXlsxAsync(Stream output, ReportSheetDto template, IAsyncEnumerable<ReportSheetRowDto> rows, CancellationToken ct)
        {
            var captured = new List<ReportSheetRowDto>();
            await new ReportXlsxExportService().WriteXlsxAsync(output, template, Capture(), ct);
            Sheet = template with { Rows = captured };
            async IAsyncEnumerable<ReportSheetRowDto> Capture()
            {
                await foreach (var row in rows.WithCancellation(ct)) { captured.Add(row); yield return row; }
            }
        }
    }
