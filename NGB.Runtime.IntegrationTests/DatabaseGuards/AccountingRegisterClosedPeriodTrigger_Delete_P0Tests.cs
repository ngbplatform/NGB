    ...
    : IntegrationTestBase(fixture)
{
    ...
    [Fact]
    public async Task Trigger_Forbids_DeleteFromRegisterMain_ForClosedPeriod()
    {
        ...
        Func<Task> act = () => DeleteRegisterRowsAsync(Fixture.ConnectionString, doc);
    ...
    [Fact]
    public async Task Trigger_Allows_DeleteFromRegisterMain_ForOpenPeriod()
    {
        using var host = IntegrationHostFactory.Create(Fixture.ConnectionString);
        ...
        await DeleteRegisterRowsAsync(Fixture.ConnectionString, doc);
        (await CountRegisterRowsAsync(Fixture.ConnectionString, doc)).Should().Be(0);
    }
    ...
    private static async Task DeleteRegisterRowsAsync(string cs, Guid documentId)
    {
        await using var conn = new NpgsqlConnection(cs);
        await conn.OpenAsync(CancellationToken.None);
