...
public sealed class TradeCommandPaletteSearchServiceFullCoverageTests
{
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    ...
    public async Task SearchAsync_AllProviders_ReturnsRankedDomainResultsAndCachesMetadata()
    {
        ...
        fixture.SetReports(AllReportIconCodes()
            ...
            .Append(new ReportDefinitionDto(TradeCodes.DashboardOverviewReport, "Match empty description", null, " "))
        ...
        fixture.SetDocumentHits(
            new DocumentLookupDto(documentId, TradeCodes.Item, "Match display", DocumentStatus.Draft, false, "  MATCH-001  "),
            new DocumentLookupDto(fallbackDocumentId, TradeCodes.Warehouse, "   ", DocumentStatus.Posted, false, "match-posted"),
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Item, "match same", DocumentStatus.Draft, false, "match same"),
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Item, "match empty number", DocumentStatus.Draft, false, "   "),
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Party, "Match party", DocumentStatus.MarkedForDeletion, true),
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.PriceType, "Match price", (DocumentStatus)99, false),
            new DocumentLookupDto(Guid.NewGuid(), "trd.unknown", "Match unknown", DocumentStatus.Draft, false));
        fixture.SetCatalogHits(
            new CatalogLookupDto(catalogId, TradeCodes.Item, " Match catalog ", false),
            new CatalogLookupDto(Guid.NewGuid(), TradeCodes.Warehouse, "Match deleted", true),
            new CatalogLookupDto(Guid.NewGuid(), TradeCodes.Item, null, false),
            new CatalogLookupDto(Guid.NewGuid(), TradeCodes.Party, "   ", false),
            new CatalogLookupDto(Guid.NewGuid(), "trd.unknown", "Match unknown", false));

        var context = new CommandPaletteSearchContextDto(
            EntityType: "other",
            DocumentType: TradeCodes.Item.ToUpperInvariant(),
            CatalogType: TradeCodes.Item.ToUpperInvariant());
        ...
        first.Groups.Single(x => x.Code == "documents").Items.Should().Contain(x =>
            x.Key == $"document:{TradeCodes.Item}:{documentId}"
            && x.Title == "trd.item MATCH-001"
            && x.Subtitle == "Match display · Draft"
            && x.Icon == "custom-icon"
            && x.Status == "draft"
            && x.Score == 0.95m);
        first.Groups.Single(x => x.Code == "documents").Items.Should().Contain(x =>
            x.Key == $"document:{TradeCodes.Warehouse}:{fallbackDocumentId}"
            && x.Subtitle == "Posted");
        ...
        first.Groups.Single(x => x.Code == "catalogs").Items.Should().Contain(x =>
            x.Key == $"catalog:{TradeCodes.Item}:{catalogId}"
        ...
        first.Groups.Single(x => x.Code == "reports").Items.Should().Contain(x => x.Icon == "git-merge");
        first.Groups.Single(x => x.Code == "reports").Items.Should().Contain(x => x.Icon == "tag");
    ...
    {
        using var fixture = new Fixture();
        fixture.SetDocumentMetadata(DocumentMetadata(TradeCodes.Item));
        fixture.SetDocumentHits(new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Item, display, DocumentStatus.Draft, false));
    ...
    {
        using var fixture = new Fixture();
        fixture.SetDocumentMetadata(DocumentMetadata(TradeCodes.Item, label: "Merchandise"));
        fixture.SetDocumentHits(
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Item, null, DocumentStatus.Draft, false),
            new DocumentLookupDto(Guid.NewGuid(), TradeCodes.Item, null, DocumentStatus.Draft, false, " "));
        fixture.SetCatalogMetadata(CatalogMetadata(TradeCodes.Item));
        var catalogId = Guid.NewGuid();
        fixture.SetCatalogHits(new CatalogLookupDto(catalogId, TradeCodes.Item, " ", false));
        fixture.SetReports(new ReportDefinitionDto(TradeCodes.DashboardOverviewReport, "sku", " ", null));

        var result = await fixture.Sut.SearchAsync(new CommandPaletteSearchRequestDto("sku"), CancellationToken.None);

        result.Groups.Single(x => x.Code == "documents").Items.Should().OnlyContain(x =>
            x.Title.StartsWith("Merchandise ", StringComparison.Ordinal));
        result.Groups.Single(x => x.Code == "catalogs").Items.Single().Title.Should().Be($"{TradeCodes.Item} {catalogId}");
    ...
    public async Task SearchAsync_ProviderFailureIsIsolatedAndUncancelledCancellationIsLogged()
    {
        ...
        fixture.SetReports(new ReportDefinitionDto(TradeCodes.DashboardOverviewReport, "match"));
    ...
    {
        yield return TradeCodes.Item;
        yield return TradeCodes.Warehouse;
        yield return TradeCodes.Party;
        yield return TradeCodes.PriceType;
        yield return TradeCodes.PurchaseReceipt;
        yield return TradeCodes.SalesInvoice;
        yield return TradeCodes.InventoryTransfer;
        yield return TradeCodes.InventoryAdjustment;
        yield return TradeCodes.CustomerReturn;
        yield return TradeCodes.VendorReturn;
        yield return TradeCodes.CustomerPayment;
        yield return TradeCodes.VendorPayment;
        yield return TradeCodes.ItemPriceUpdate;
    }
    ...
    {
        yield return TradeCodes.InventoryBalancesReport;
        yield return TradeCodes.InventoryMovementsReport;
        yield return TradeCodes.CurrentItemPricesReport;
        yield return TradeCodes.SalesByItemReport;
        yield return TradeCodes.SalesByCustomerReport;
        yield return TradeCodes.PurchasesByVendorReport;
        yield return TradeCodes.DashboardOverviewReport;
    }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        public Fixture()
        {
            ...
            Sut = new TradeCommandPaletteSearchService(
                Documents.Object,
                Catalogs.Object,
                Reports.Object,
                _cache,
        ...
        public TradeCommandPaletteSearchService Sut { get; }
